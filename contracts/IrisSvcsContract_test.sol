...
import "./strings.sol";
 ...
 contract IrisSvcsContracts{
    ...
    function svc_bind_update(string _svc_cd, address _bind_address, string _action,
                      uint _fee, string _scope, string _block_type){
        ...
        bind.bind_addr = _bind_address;
        bind.action = _action;
        bind.fee = _fee;
        bind.scope = _scope;
        bind.block_type = _block_type;
        bind.updatetime = block.timestamp;
        addr2fee[_bind_address] = _fee;
     ...
     function test(uint a,uint b) returns(string,string,string){
         string[50] memory str_arr;
         string memory output;
         uint c;
         str_arr[c++] = int2string(a);
         str_arr[c++] = ",";
         str_arr[c++] = int2string(b);
         var slice_arr = new strings.slice[](50);
         for(uint i = 0;i < c;i++){
             slice_arr[i] = str_arr[i].toSlice();
         }
     ...
     function test1(string a,string b) returns(string){
         string[50] memory str_arr;
         string memory output;
         uint c;
         str_arr[c++] = a;
     ...
     function test2(uint a,uint b) returns(string){
         string[50] memory str_arr;
         string memory output;
         uint c;
         str_arr[c++] = int2string(a);
        ...
        //  bytes memory result;
        //  for(i =0;i < c;i++){
        //     bytes memory aaa = bytes(str_arr[i]);
        //     for (uint j = 0; j < aaa.length; j++) result[k++] = aaa[j];
        //  }
        //  output = "".toSlice().join(slice_arr);
